...
public class DeviceManager
{
	...
	{
		DeviceManager deviceClass = new DeviceManager();
		...
		int prevMaxHumidityThreshold = Integer.MIN_VALUE;
		...
		try
		{
			 ...
			 * 
			 * parameters: a. name of configuration file (downloaded from IoT
			 * CS) b. pwd to protect it
			 */
			 ...
			 */
			virtualHumiditySensor.setOnChange(new VirtualDevice.ChangeCallback<VirtualDevice>() {
				public void onChange(VirtualDevice.ChangeEvent<VirtualDevice> event)
				{
					...
					boolean first = true;
					for (VirtualDevice.NamedValue<?> namedValue = namedValues; namedValue != null; namedValue = namedValue
							.next())
					{
						...
						if (MAX_THRESHOLD_ATTRIBUTE.equals(attribute))
						{
							...
							/*
							 * After this callback returns without an exception,
							 * the virtual device attribute will be updated
							 * locally and on the server.
							 */
						} else
						{
							msg.append("\"").append(attribute).append("\" not implemented");
						}
					}
			...
			while (true)
			{
				...
				// Send humidityThreshold if it has changed or is not the
				// default
				int humidityThreshold = sensor.getMaxThreshold();
				if (prevMaxHumidityThreshold != humidityThreshold || humidityThreshold != 80)
				{
					prevMaxHumidityThreshold = humidityThreshold;

					consoleMessage.append(",\"").append(MAX_THRESHOLD_ATTRIBUTE).append("\"=")
							.append(humidityThreshold);

					virtualHumiditySensor.set(MAX_THRESHOLD_ATTRIBUTE, humidityThreshold);
				}
